...
router.get("/", validateRequest, async (req: Request, res: Response) => {
  const {
    latitude = "",
    longitude = "",
  } = req.query as GetLocationNameFromCoordinatesQuery;
  ...
  try {
    ...
    const validResult = result.data["results"].find(
      (element: any) => element["formatted_address"]
    ) as any;
    if (validResult) {
...
router.post("/delete", validateRequest, async (req: Request, res: Response) => {
  const { id = "" } = req.body;
  if (id.length === 0) {
    res.status(400).send();
    return;
  ...
  try {
    await Database.Location.delete(locationId, payload.sub);
    res.status(200).send();
  } catch (err) {
    console.log(err);
    res.status(500).send();
